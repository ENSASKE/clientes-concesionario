...
@Component("reporteconcesionario")
public class ReporteConcesionarioPdfView extends AbstractPdfView {
	...
	protected void buildPdfDocument(Map<String, Object> model, Document document, PdfWriter writer,
		...
		cell = new PdfPCell(new Phrase("Repore Clientes por Concesionario"));
		...
		tabla2.setWidths(new float [] {1, 1, 2.5f, 1.3f});
		tabla2.addCell("Nombre");
		tabla2.addCell("Apellido");
		tabla2.addCell("Correo");
		tabla2.addCell("Concesionario");
		
		for(Cliente item: clienteService.findAllConcesionario()) {
			tabla2.addCell(item.getNombre());
			tabla2.addCell(item.getApellido());
			tabla2.addCell(item.getEmail());
			
			cell = new PdfPCell(new Phrase(item.getConcesionario().getNombre()));
	    ...
	    tabla2.addCell(cell);
            int total = clienteService.findAllConcesionario().size();
